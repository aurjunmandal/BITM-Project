...
namespace UniversityManagementSystemApp.Controllers
{
    public class EnrollCourseController : Controller
    {
        ...
        public ActionResult Create([Bind(Include="EnrollCourseId,StudentId,CourseId,Date")] EnrollCourse enrollcourse)
        {
            ...
            "Hi, " + db.Students.Where(s => s.StudentId == enrollcourse.StudentId).Select(s => s.Name).Single() + "! you already enrolled this course on " + db.EnrollCourses.Include(ec => ec.Course).Include(ec => ec.Student).Where(ec => ec.StudentId == enrollcourse.StudentId).Where(ec => ec.CourseId == enrollcourse.CourseId).Select(es => es.Date).Single() + ", try different course";
